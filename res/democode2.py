...
def create_train_test_data(dataset, look_back):
    ...
    trainX, trainY, testX, testY = datasetX[0:train_size, :], datasetY[0:train_size], datasetX[train_size:len(
...
def build_model(layers, seq_len):  # layers [1,50,100,1]
    ...
    model.add(LSTM(layers[1], input_shape=(
        seq_len, layers[0]), return_sequences=True))
...
def plot_results(predicted_data, true_data, predicted_next):
    PredictionNextX = [i for i in range(
        len(predicted_data), (len(predicted_data)+len(predicted_next)))]
    fig = plt.figure(facecolor='white', figsize=(10, 5))
    #ax = fig.add_subplot(111)
    #ax.plot(true_data, label='True Data')
    plt.plot(true_data, color="black", label='True Data')
    plt.plot(predicted_data, color="blue", label='Prediction')
    plt.plot(PredictionNextX, predicted_next,
             color="red", label='PredictionNext')
...
def predict_next(model, scaler, dataset_pre, look_back, next_num):
    dataset_pre = scaler.transform(
        dataset[len(dataset)-look_back:len(dataset), 0])
    next_predicted_list = []
...
if __name__ == '__main__':
    ...
    scaler, trainX, trainY, testX, testY = create_train_test_data(
        dataset, seq_len)
    ...
    # 模型训练
    history = model.fit(trainX, trainY, batch_size=batchSize,
                        epochs=nb_epoch, validation_split=0.05, verbose=1, shuffle=True)
    ...
    # 预测未来数据
    next_predicted_list = predict_next(
        model, scaler, dataset, seq_len, future_predict_num)
    # 结果展示
    plot_results(testPredict, testY[0], next_predicted_list)
    # 保存模型
    hyperparams_name = str(seq_len)+"-"+str(lstmFirstLayer) + \
        "-"+str(lstmSecondLayer)+"-"+str(testScore)
...
作者：chaochaopang0 
来源：CSDN 
原文：https://blog.csdn.net/chaochaopang0/article/details/80266155 
